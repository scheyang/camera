 ...
 *
 * Utility routines for yuyv to bmp ,rgb interaction
 */
...
static void  yuv2rgb565(const int y, const int u, const int v, unsigned short *prgb565)
{
	...
	#define CLAMP(x) if (x < 0) x = 0 ; else if (x > 255) x = 255
	 ...
	 CLAMP(b);
	#undef CLAMP
	 *prgb565 = (unsigned short)(((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3));

}

static unsigned short * ConvertYUYVtoRGB565(const void *yuyv_data, void *rgb565_data, const unsigned int w, const unsigned int h)
{

	const unsigned char *src = (unsigned char *)yuyv_data;
	unsigned short *camera_rgb = (unsigned short *)rgb565_data;

	unsigned int  i,j;
	for (i = 0, j = 0; i < w * h * 2; i += 4, j += 2) {
		int y, u, v;
		...
		camera_rgb[j + 0] = rgb565;

		y = src[i + 2];
		yuv2rgb565(y, u, v, &rgb565);
		camera_rgb[j + 1] = rgb565;
...
int yuyvtorgb16(const unsigned char *yuyv,const unsigned int w,const unsigned int h,unsigned char **rgb16)
{

	//if *rgb16 is NULL,Ôò¿ª±Ù¿Õ¼ä
	if(*rgb16==NULL){
		puts("before request the rgb16 buf");
		*rgb16 = calloc( 1, w*h*2);
		if(*rgb16==NULL){
			printf("calloc rgb16  failed\n");	
			return -1;
		}
	}
	ConvertYUYVtoRGB565(yuyv, *rgb16,w,h);
